...
def fetch_hh(text, schedule=None, period=14, max_pages=1):
    return bot.fetch_hh_paginated(text, period=period, schedule=schedule, max_pages=max_pages)
        ...
        try:
            ...
            for role, rules in PROFILES.items():
                for orq in or_batches(rules["keywords"], size=6):
                    ...
                    check_remote_stop()
                    items = fetch_hh(orq, schedule="remote", period=7, max_pages=3)
                    if items:
